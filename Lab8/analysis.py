...
def normalize_columns_separately(headers, data, datatype =0):
	data.get_num_headers(datatype)
	selected = data.getNumCol(headers)
...
def normalize_columns_together(headers, data, datatype = 0):
	data.get_num_headers(datatype)
	selected = data.getNumCol(headers)
...
# Returns a matrix of the distance between the closest mean and each point
...
def kmeans_algorithm(A, means, metric=0):
...
def kmeans(d, headers, K, metric, whiten=True ): 
...
def getkobj(d, headers, K, metric, whiten):
	codebook, codes, errors = kmeans(d, headers, K, metric, whiten)
	kobj = data.ClusterData(codebook, codes, errors)
	return kobj
